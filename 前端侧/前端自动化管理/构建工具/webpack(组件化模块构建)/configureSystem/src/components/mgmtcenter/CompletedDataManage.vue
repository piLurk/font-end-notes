				...
				<li>
					...
					<div class="inpbox">
						<el-select v-model="filterForm.orderState" placeholder="请选择">
							<el-option v-for="(item,i) in orderStateOptions" :disabled="item.value !== 11" :key="i" :label="item.label" :value="item.value">
							</el-option>
						</el-select>
					</div>
				</li>
											...
											<div class="subDiv">
												<table class="subTable" width="100%" border="0" cellpadding="0" cellspacing="0">
													...
													<tr v-else v-for="(work,index) in planGroup[item.pkId]" :key="index">
														...
														<td>
															<p class="typeControl">
																<a v-if="hasPermission('PZ_editAllot')" class="w-btn" @click="operate('completedDataEdit','edit','竣工数据编辑',work)">
																	<i></i>编辑</a>
															</p>
														</td>
													</tr>
...
import { getWorkOrders } from "../../api/projectdelivery";
import {
  orderTypeOptions,
  houseTypeOptions,
  orderStateOptions,
