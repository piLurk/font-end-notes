    ...
    <div class="modbox">
      ...
      <div v-if="providerType != '家居' && providerType != '宽带'" class="tableBox mgtb15">
        <table class="modtable" width="100%" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr v-for="(secondItem,secindex) in baseInfo.particularsVos" :key="secondItem.pkId" :class="rowClass[secindex]">
            <td width="200" v-if="mergrRule[secindex]" :rowspan="mergrRule[secindex]" class="function-type">{{secondItem.functionType}}</td>
            <td width="200">{{secondItem.sortName}}</td>
            <td width="200">{{secondItem.goodsName}}</td>
            <td width="150">{{secondItem.unit}}</td>
            <td width="150">{{secondItem.unitPrice}}</td>
            <td width="150">{{secondItem.number}}</td>
            <td width="150" v-if="params.oper==='check' || params.oper==='edit'" style="padding:0 15px;"><input type="text" v-model.trim="secondItem.actualNum" placeholder="请输入" class="inp" :class="{'error-inp': secondItem.actualNum===''}" /></td>
            <td width="150" v-else>{{secondItem.actualNum}}</td>
          </tr>
        </table>
      </div>
      ...
      <div v-if="providerType == '家居'" class="tableBox mgtb15">
        <table class="modtable" width="100%" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr v-for="(secondItem,secindex) in baseInfo.particularsVos" :key="secondItem.pkId">
            <td width="200" v-if="mergrRule[secindex]" :rowspan="mergrRule[secindex]" class="function-type">{{secondItem.functionType}}</td>
            <td width="200">{{secondItem.sortName}}</td>
            <td width="200">{{secondItem.goodsName}}</td>
            <td width="150">{{secondItem.unit}}</td>
            <td width="150">{{secondItem.unitPrice}}</td>
            <td width="150">{{secondItem.number}}</td>
            <td width="150" v-if="params.oper==='check' || params.oper==='edit'" style="padding:0 15px;"><input type="text" v-model.trim="secondItem.actualNum" placeholder="请输入" class="inp" /></td>
            <td width="150" v-else>{{secondItem.actualNum}}</td>
      ...
      <div class="btnbar" v-if="params.oper==='check' || params.oper==='edit'">
        <input type="button" :value="params.oper === 'check' ? '完成派工验收':'确认' " class="sureDbtn" @click="accept">
      </div>
    </div>
...
import { getCheckOrders, getCheckedOrders, checkOrders, editOrders } from '../../api/projectdelivery'

import { getSortGoods, getGoodsPrice } from '../../api/basicdatamgmt'
import BandwidthForm from './bandwidthForm.vue'
...
export default {
  ...
  data() {
    return {
      ...
      rules3: {
        ...
        exceedContent: [{ max: 500, message: '长度在 500 个字符以内', trigger: 'blur' }]
      },
      // sendTime: {
      //   actualDate: '' //实际到达时间
      // },
  ...
  methods: {
    ...
    calcEndDate() {
      ...
      let result = _startDate.setMonth(oldMonth + monthAdd) - 24 * 60 * 60 * 1000
    ...
    getCheckedOrdersInfo(pkId) {
      getCheckedOrders(pkId, data => {
        //this.sortList(data, 'particularsVos')
        const _list = this.getRoomList(data['particularsVos']);
        this.roomList = _list;
        ...
        if (this.params.oper === 'edit') {
          ...
          this.loadBandwidthOptions(data.bandwidthVo.bandwidthType, data.bandwidthVo.bandwidthName)
        }
      })
    },
    ...
    validate(cb) {
      const mainInfo = this.providerType == '宽带' ? 'bandwidthInfo' : 'mainInfo'
    ...
    accept() {
      this.validate(() => {
        const msg = {
          check: ['确认完成派工验收?', '验收成功', '验收失败', '已取消验收'],
          edit: ['确认完成派工验收?', '验收成功', '验收失败', '已取消验收']
        }
        const handler = {
          check: checkOrders,
          edit: editOrders
        }
        var _msg = msg[this.params.oper]
        var _handler = handler[this.params.oper]
