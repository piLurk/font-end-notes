...
export default {
  name: 'editPlanTable',
  props: ['parentItem', 'parentIndex', 'contractNumber', 'tabsIndex', 'update'],
  components: {
  ...
  computed: {
    ...
    l2SortId() {
      return this.params.oper === 'create' ? this.parentItem.pkId : this.parentItem.sortId
    },
    type() {
  ...
  methods: {
    ...
    initPage() {
      ...
      const _init = data => {
        if (!data) {
          if (this.type === 1) {
            ...
            data = {
              deployParticularMap: {
                goods: Array.apply(null, Array(this.roomList.length)).map(_ => []),
                rooms: [...this.roomList]
              }
            }
          }
      ...
      if (this.params.oper === 'create') {
        ...
        if (this.parentItem.pkId) {
          getConfPlanDetail(this.parentItem.pkId, _init)
        } else {
          _init()
        }
      }
